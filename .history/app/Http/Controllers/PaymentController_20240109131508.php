...
public function convertAndAddToWallet(Request $request)
{
    ...
    if (!$userWallet) {
        ...
        $userWallet->total_balance = 0; // Initialize the balance if creating a new wallet
        $userWallet->points_reward = 0; // Initialize the points if creating a new wallet
    }

    $points = $userWallet->points_reward;
    ...
    if ($rupees > 0) {
        ...
        $userWallet->points_reward -= $convertedPoints;
        $userWallet->points_reward = max(0, $userWallet->points_reward); // Ensure points don't go negative
        $userWallet = new Wallet();
        $userWallet->user_id = $userId;
       // Initialize the balance if creating a new wallet
